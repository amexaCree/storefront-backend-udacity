...
export class ProductStore {
    ...
    categoryIndex = async (category: string):Promise<Product[]> => {
        try {
            ...
            throw new Error(`Could not find products in '${category}' category. ${err}`)
        }
    }
    ...
    async showOrderProducts (orderId: string): Promise<Product[]> {
        // TODO - first check that order (orderId) is still active (not completed.)
        try {
            const conn = await Client.connect()
            // let sql = "SELECT * FROM order_products WHERE order_id=$1"
            let sql = "SELECT products.id, products.name, products.price, order_products.quantity, order_products.order_id FROM order_products INNER JOIN products ON products.id = order_products.product_id WHERE order_id=$1"
